...
@ManagedBean
@ViewScoped
public class CtRentasRecaudacionesDiarias implements Serializable {
    ...
    public CtRentasRecaudacionesDiarias() {
    ...
    public void obtenerTransacciones() {
        try {
            ...
            setLstTransacciones(FTransaccion.obtenerTransaccionesDadoFecha(sqlDate));
            setTotal(0);
    ...
    public void obtenerTransaccionesDadoFecha() {
        try {
            setLstTransacciones(FTransaccion.obtenerTransaccionesDadoFecha(Util.sqlDate(getFecha())));
            setTotal(0);
            for (int i = 0; i < lstTransacciones.size(); i++) {
                setTotal(getTotal() + lstTransacciones.get(i).getValorTransaccion());
            }
    ...
    public void obtenerTransaccionesDadoRangoFecha() {
        try {
            setLstTransacciones(FTransaccion.obtenerTransaccionesDadoRangoFechas(Util.sqlDate(getFechaInicio()), Util.sqlDate(getFechaFin())));
            setTotal(0);
            for (int i = 0; i < lstTransacciones.size(); i++) {
                setTotal(getTotal() + lstTransacciones.get(i).getValorTransaccion());
            }
