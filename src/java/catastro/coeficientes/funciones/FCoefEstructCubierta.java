...
public class FCoefEstructCubierta {
    ...
    public static List<CoefCubierta> obtenerCoeficientes() throws Exception {
     ...
     public static String registrarCoeficiente(CoefCubierta coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_insertar_coef_cubierta(?,?,?)";
    ...
    public static String actualizarCoeficiente(CoefCubierta coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_actualizar_coef_cubierta(?,?,?,?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setString(1, coeficiente.getCoeficiente());
            prstm.setInt(2, coeficiente.getCodigo());
            prstm.setDouble(3, coeficiente.getValorCoeficiente());
            prstm.setInt(4, coeficiente.getIdCoefCubierta());
            resultSet = accesoDatos.ejecutaPrepared(prstm);
            if (resultSet.next()) {
                respuesta = resultSet.getString(1);
                return respuesta;
    ...
    public static String eliminarCoeficiente(CoefCubierta coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_desactivar_coef_cubierta(?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setInt(1, coeficiente.getIdCoefCubierta());
            resultSet = accesoDatos.ejecutaPrepared(prstm);
            if (resultSet.next()) {
                respuesta = resultSet.getString(1);
                return respuesta;
