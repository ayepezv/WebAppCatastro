...
public class FCoefRevExt {

    public static List<CoefRevExterior> obtenerCoefAcabadosRevExterior() throws Exception {
        List<CoefRevExterior> lst = new ArrayList<>();
        AccesoDatos accesoDatos;
        CoefRevExterior coeficiente;
        ResultSet resultSet;
        String sql;
        try {
            accesoDatos = new AccesoDatos();
            sql = "SELECT * from catastro.f_coefiencientes_rev_ext();";
            resultSet = accesoDatos.ejecutaQuery(sql);
            while (resultSet.next()) {
                coeficiente = new CoefRevExterior();
                coeficiente.setIdCoeficiente(resultSet.getInt("sr_id_coef_reve"));
    ...
    public static String registrarCoeficiente(CoefRevExterior coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_insertar_coef_acab_rev_ext(?,?,?)";
    ...
    public static String actualizarCoeficiente(CoefRevExterior coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_actualizar_coef_acab_rev_ext(?,?,?,?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setString(1, coeficiente.getCoeficiente());
            prstm.setInt(2, coeficiente.getCodigo());
            prstm.setDouble(3, coeficiente.getValorCoeficiente());
    ...
    public static String eliminarCoeficiente(CoefRevExterior coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_desactivar_coef_acab_rev_ext(?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
