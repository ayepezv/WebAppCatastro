...
public class FCoefAlcantarillado {

    public static List<CoefAlcantarillado> obtenerCoeficientes() throws Exception {
        List<CoefAlcantarillado> lst = new ArrayList<>();
        AccesoDatos accesoDatos;
        CoefAlcantarillado coeficiente;
        ResultSet resultSet;
        String sql;
        try {
            accesoDatos = new AccesoDatos();
            sql = "SELECT * from catastro.f_coefiencientes_alcantarillado();";
            resultSet = accesoDatos.ejecutaQuery(sql);
            while (resultSet.next()) {
                coeficiente = new CoefAlcantarillado();
    ...
    public static String registrarCoeficiente(CoefAlcantarillado coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_insertar_coef_alcantarillado(?,?,?)";
    ...
    public static String actualizarCoeficiente(CoefAlcantarillado coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_actualizar_coef_alcantarillado(?,?,?,?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setString(1, coeficiente.getCoeficiente());
            prstm.setInt(2, coeficiente.getCodigo());
            prstm.setDouble(3, coeficiente.getValorCoeficiente());
    ...
    public static String eliminarCoeficiente(CoefAlcantarillado coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_desactivar_coef_alcantarillado(?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
