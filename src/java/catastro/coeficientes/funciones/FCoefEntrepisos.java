...
public class FCoefEntrepisos {
    ...
    public static List<CoefEntrepisos> obtenerCoeficientes() throws Exception {
     ...
     public static String registrarCoeficiente(CoefEntrepisos coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_insertar_coef_entrepisos(?,?,?)";
    ...
    public static String actualizarCoeficiente(CoefEntrepisos coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_actualizar_coef_entrepisos(?,?,?,?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setString(1, coeficiente.getCoeficiente());
            prstm.setInt(2, coeficiente.getCodigo());
            prstm.setDouble(3, coeficiente.getValorCoeficiente());
            prstm.setInt(4, coeficiente.getIdCoefEntrepisos());
            resultSet = accesoDatos.ejecutaPrepared(prstm);
            if (resultSet.next()) {
                respuesta = resultSet.getString(1);
                return respuesta;
    ...
    public static String eliminarCoeficiente(CoefEntrepisos coeficiente) throws Exception {
        String respuesta;
        AccesoDatos accesoDatos;
        String sql;
        PreparedStatement prstm;
        ...
        try {
            accesoDatos = new AccesoDatos();
            sql = "select * from catastro.f_desactivar_coef_entrepisos(?)";
            prstm = accesoDatos.creaPreparedSmt(sql);
            prstm.setInt(1, coeficiente.getIdCoefEntrepisos());
            resultSet = accesoDatos.ejecutaPrepared(prstm);
            if (resultSet.next()) {
                respuesta = resultSet.getString(1);
                return respuesta;
